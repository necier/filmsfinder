  ...
  <div class="container">
    ...
    <form @submit.prevent="submitForm" class="col-md-6">
      ...
      <div class="mb-3">
        ...
        <input v-model="form.imdbLink" type="url" class="form-control" placeholder="https://www.imdb.com/title/..." />
      </div>
...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        await http.post('/movies', this.form);
        this.$router.push('/');
        alert('添加成功');
      } catch (e) {
        alert('添加失败：' + (e.response?.data || e.message));
      }
    }
  }
};
