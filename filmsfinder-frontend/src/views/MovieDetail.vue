...
<template>
  <div class="movie-detail-page">
    <!-- 动态模糊背景 -->
    <div class="movie-detail-background" :style="backgroundStyle"></div>
    <!-- 半透明叠加层 -->
    ...
    <div v-if="movie" class="movie-detail-content">
      ...
      <div class="row mb-5">
        ...
        <div class="col-md-8 col-lg-9 detail-info-panel">
          <!-- 主副标题 -->
          <h1 class="movie-title">{{ movie.name }}</h1>
          <h2 class="movie-subtitle" v-if="movie.chineseName">{{ movie.chineseName }}</h2>

          <!-- 核心信息标签 -->
          <div class="meta-info mb-4">
            <span class="badge">{{ movie.releaseYear }}</span>
            <span class="badge">{{ movie.type }}</span>
            <span class="badge">{{ movie.duration }} 分钟</span>
          ...
          <!-- IMDB 链接 -->
          <div class="imdb-section" v-if="movie.imdbLink">
            <a :href="movie.imdbLink" target="_blank" class="btn btn-dark btn-sm">
              前往 IMDB
            </a>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    backgroundStyle() {
      const imageUrl = this.movie?.posterUrl || this.placeholder;
      return {
